...
if page == "\U0001f50d Scanner":
    ...
    if st.session_state.results is not None:
        ...
        tablo_df = pd.DataFrame([{
            ...
            'C':          r['C'],
            'D':          r['D'],
            'Faal.Kar\u0131':  r['Faal.Kar\u0131'],
            'Piy.De\u011feri': r['Piy.De\u011feri'],
            'FK/PD%':     r.get('FK/PD%', '-'),
            'B\u00fcy\u00fcme%': r.get('B\u00fcy\u00fcme%', '-'),
        } for r in goster])
        ...
        edited = st.data_editor(
            tablo_df,
            column_config={
                '\u2b50': st.column_config.CheckboxColumn('\u2b50', width='small', help='Takip listesine ekle'),
                'Kod':        st.column_config.TextColumn('Kod', width='small'),
                'Sekt\u00f6r': st.column_config.TextColumn('Sekt\u00f6r', width='medium'),
                ...
                'Faal.Kar\u0131':  st.column_config.TextColumn('Faal.Kar\u0131', width='small'),
                'Piy.De\u011feri': st.column_config.TextColumn('Piy.De\u011feri', width='small'),
                'FK/PD%':     st.column_config.TextColumn('FK/PD%', width='small'),
                'B\u00fcy\u00fcme%': st.column_config.TextColumn('B\u00fcy\u00fcme%', width='small'),
            },
            disabled=['Kod','Sekt\u00f6r','Puan','Karar','A','B','C','D',
                      'Faal.Kar\u0131','Piy.De\u011feri','FK/PD%','B\u00fcy\u00fcme%'],
            hide_index=True,
            use_container_width=True,
            height=min(40 + len(goster) * 35, 600),
            key='scanner_table',
        )

        # Checkbox değişikliklerini watchlist'e yansıt
        for i, row in edited.iterrows():
            ...
            if istek and not in_wl:
                st.session_state.watchlist[kod] = {
                    'puan': r_data['Puan'], 'karar': r_data['Karar'],
                    'sektor': r_data['Sekt\u00f6r'],
